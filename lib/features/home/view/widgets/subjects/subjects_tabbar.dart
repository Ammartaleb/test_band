...
class SubjectsTabBar extends StatelessWidget {
  final TabController? tabController;
  final int currentIndex;
  final Function(int) onTapFunction;
  final List<SubjectModel> subjects;

  const SubjectsTabBar({
    super.key,
    this.tabController,
...
class SubjectsTabBarItem extends StatelessWidget {
  final bool isSelected;
  final SubjectModel subject;
  const SubjectsTabBarItem({
    super.key,
  ...
  Widget build(BuildContext context) {
    return Tab(
      child: Container(
        ...
        child: Text(subject.name ?? "Dummy Subject"),
      ),
    );
  }
}
