...
class RePasswordInputFieldWidget extends StatelessWidget {
  const RePasswordInputFieldWidget({super.key});

  @override
  Widget build(BuildContext context) {
    return PasswordInputFieldWidget(
      textEditingController: context.read<AuthCubit>().rePasswordController,
      text: "تأكيد كلمة المرور",
      onValidate: (input) => getIt<AppValidators>().validateRePassword(
        password: context.read<AuthCubit>().passwordController.text,
        rePassword: context.read<AuthCubit>().rePasswordController.text,
      ),
    );
  }
}
